...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        ...
        // Handle paginated response structure { activities, total, page, limit }
  ...
  const recentActivity = useMemo(() => {
    return activities.slice(0, 4).map(activity => ({
      id: activity.id,
      title: activity.action.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()),
      student: activity.email?.split('@')[0] || 'User', // Fallback display name
      time: new Date(activity.createdAt).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }),
      status: activity.action.includes('approved') ? 'Approved' : activity.action.includes('rejected') ? 'Rejected' : 'Pending'
    }));
  }, [activities]);
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '1rem', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          <div style={{ padding: '1.5rem', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button
              ...
              style={{ color: '#15803d', fontSize: '0.875rem', fontWeight: 600, background: 'none', border: 'none', cursor: 'pointer' }}
            >
              View all
            </button>
          </div>
