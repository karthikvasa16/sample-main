...
exports.sendCustomEmail = async (email, subject, message) => {
    ...
    html: `
      ...
      <head>
        ...
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; }
          .header { background: #064e3b; color: white; padding: 20px; text-align: center; border-radius: 8px 8px 0 0; }
      ...
      <body>
        <div class="container">
          <div class="header">
            <h2 style="margin:0;">Message from Kubera Admin</h2>
          </div>
          <div class="content">
            <pre>${message}</pre>
            <p style="margin-top: 30px; font-size: 0.9em; color: #6b7280;">
              This is an important notification from the Kubera administration team.
            </p>
          </div>
          <div class="footer">
            <p>&copy; ${new Date().getFullYear()} Kubera. All rights reserved.</p>
  ...
  try {
    const info = await transporter.sendMail(mailOptions);
    console.log('✅ Custom email sent:', info.messageId || 'dev mode');
    return true;
  } catch (error) {
