...
const { User, PasswordResetToken, EmailVerificationToken, AccountActivityLog, StudentLead, LoanApplication, StudentDocument } = require('./models');
...
const requireAdminOrSuper = async (req, res, next) => {
  try {
    ...
    req.authenticatedUser = requestingUser;
    next();
  } catch (error) {
...
app.post('/api/leads', async (req, res) => {
  try {
    ...
    // Ensure StudentLead table exists
    try {
      await StudentLead.sync({ alter: false });
      console.log('✅ StudentLead table verified');
    } catch (syncError) {
      console.error('❌ Table sync error:', syncError);
      // Try to create the table if it doesn't exist
      try {
        await StudentLead.sync({ force: false });
        console.log('✅ StudentLead table created');
      } catch (createError) {
        console.error('❌ Failed to create table:', createError);
        return res.status(500).json({
          error: 'Database table error. Please contact support.',
    ...
    if (!studyCountry) missingFields.push('studyCountry');
    if (!admissionStatus) missingFields.push('admissionStatus');
    if (!intake) missingFields.push('intake');
    if (!loanRange) missingFields.push('loanRange');
    ...
    const allowedAdmissionStatuses = ['not_applied', 'applied', 'confirmed'];
    const normalizedAdmissionStatus = allowedAdmissionStatuses.includes(admissionStatus)
      ? admissionStatus
      : 'not_applied';

    // Validate fullName length before saving
    ...
    if (trimmedFullName.length > 120) {
      return res.status(400).json({
        error: 'Full name must be less than 120 characters'
      });
    }
    ...
    const defaults = {
      ...
      intake,
      universityPreference: (universityPreference && universityPreference.trim()) || null,
      loanRange,
    ...
    console.log('💾 Attempting to save lead:', {
      ...
      defaults: {
        ...
        universityPreference: defaults.universityPreference || 'null'
      }
    });

    let lead, created;
    try {
      [lead, created] = await StudentLead.findOrCreate({
        where: { email: normalizedEmail },
        defaults
      });
      console.log(`✅ Lead ${created ? 'created' : 'updated'}:`, {
        id: lead.id,
        email: lead.email,
      ...
      if (dbError.errors) {
        console.error('Validation errors:', dbError.errors.map(e => ({
          field: e.path,
          message: e.message,
          value: e.value
        })));
    ...
    if (!created) {
      ...
      lead.studyCountry = studyCountry;
      lead.admissionStatus = normalizedAdmissionStatus;
      lead.intake = intake;
      lead.universityPreference = (universityPreference && universityPreference.trim()) || lead.universityPreference;
      lead.loanRange = loanRange;
      lead.city = (city && city.trim()) || lead.city;
      ...
      try {
        await logAccountActivity('student_lead_created', {
          email: normalizedEmail,
          metadata: {
            studyCountry,
            admissionStatus: normalizedAdmissionStatus,
        ...
        console.error('Failed to log account activity (non-critical):', logError);
    ...
    if (created) {
      try {
        ...
        console.log('✅ Enquiry confirmation email sent to:', normalizedEmail);
      } catch (emailError) {
        // Don't fail the request if email sending fails
        console.error('⚠️ Failed to send enquiry confirmation email (non-critical):', emailError.message);
      }
    }
    ...
    const responseData = {
      ...
      lead: {
        ...
        email: lead.email,
        phone: lead.phone,
        studyCountry: lead.studyCountry,
        admissionStatus: lead.admissionStatus,
        intake: lead.intake,
        universityPreference: lead.universityPreference,
        loanRange: lead.loanRange,
        city: lead.city,
        status: lead.status,
    ...
    if (error.name === 'SequelizeDatabaseError') {
      ...
      return res.status(500).json({
        error: 'Database error. Please check server logs.',
        details: process.env.NODE_ENV !== 'production' ? error.message : undefined
      });
    }

    // Return more detailed error (always show details for debugging)
    const errorMessage = `Failed to submit enquiry: ${error.message || 'Unknown error'}`;
...
// Admin: list leads
app.get('/api/leads', authenticateToken, requireAdminOrSuper, async (req, res) => {
  try {
    const { status, studyCountry, search, page = 1, limit = 20 } = req.query;
    const where = {};

    if (status && ['new', 'contacted', 'in_progress', 'verification_sent', 'converted', 'closed'].includes(status)) {
      where.status = status;
    }

    if (studyCountry) {
      where.studyCountry = studyCountry;
    }
    ...
    if (search) {
      where[Op.or] = [
        ...
        { universityPreference: { [Op.iLike]: `%${search}%` } }
      ];
    }
    ...
    const { rows, count } = await StudentLead.findAndCountAll({
      where,
...
// Admin: update lead
app.patch('/api/leads/:id', authenticateToken, requireAdminOrSuper, async (req, res) => {
  try {
    const { id } = req.params;
    const {
      status,
    ...
    const lead = await StudentLead.findByPk(id);
    if (!lead) {
    ...
    if (universityPreference) lead.universityPreference = universityPreference;
    if (loanRange) lead.loanRange = loanRange;
    if (studyCountry) lead.studyCountry = studyCountry;
    if (admissionStatus && ['not_applied', 'applied', 'confirmed'].includes(admissionStatus)) {
      lead.admissionStatus = admissionStatus;
    }
    if (intake) lead.intake = intake;

    await lead.save();

    await logAccountActivity('student_lead_updated', {
      email: lead.email,
      metadata: {
        leadId: lead.id,
        updatedBy: req.authenticatedUser.email,
        status: lead.status
      }
    });
...
// Admin: send verification email to lead
app.post('/api/leads/:id/send-verification', authenticateToken, requireAdminOrSuper, async (req, res) => {
  try {
    const { id } = req.params;
    const lead = await StudentLead.findByPk(id);
    ...
    let user = await User.findOne({ where: { email: lead.email.toLowerCase() } });
    ...
    if (user && user.emailVerified) {
      ...
      return res.json({
        ...
        message: 'User already verified. No email sent.',
        lead
      });
    }
    ...
    if (!user) {
      user = await User.create({
        ...
        country: lead.studyCountry || null,
        role: 'student',
        emailVerified: false
      });
    } else {
      user.name = lead.fullName || user.name;
      user.phone = lead.phone || user.phone;
      user.country = lead.studyCountry || user.country;
    ...
    try {
      // Send verification email with login link included
      await emailService.sendVerificationEmail(user.email, user.name || user.email, verificationToken, true);
    } catch (emailError) {
      console.error('Failed to send verification email:', emailError);
      return res.status(500).json({ error: 'Failed to send verification email. Please check email configuration.' });
    }

    lead.status = 'verification_sent';
    lead.verificationSentAt = new Date();
    lead.verificationSentBy = req.authenticatedUser.email;
    lead.convertedUserId = user.id;
    await lead.save();

    await logAccountActivity('student_lead_verification_sent', {
      userId: user.id,
      email: user.email,
      metadata: {
        leadId: lead.id,
        sentBy: req.authenticatedUser.email
      }
    });
...
app.put('/api/auth/profile', authenticateToken, async (req, res) => {
  try {
    ...
    await logAccountActivity('user_updated_profile', {
      userId: user.id,
      email: user.email,
      metadata: {
        updatedFields: {
...
app.get('/api/superadmin/activities', authenticateToken, requireSuperAdmin, async (req, res) => {
  try {
    ...
    const { rows, count } = await AccountActivityLog.findAndCountAll({
      order: [['createdAt', 'DESC']],
      limit: limitNumber,
      offset: offset
    });
    ...
    res.json({
      activities: rows.map(formatLog),
      total: count,
      page: pageNumber,
      limit: limitNumber
    });
  } catch (error) {
    console.error('Super admin activities fetch error:', error);
...
app.post('/api/loans/apply', authenticateToken, async (req, res) => {
  try {
    const { amount, purpose, duration, cibilScore, pan, universityName, course, applicantCountry, applicantIntake, admissionStatus } = req.body;
    const user = await User.findByPk(req.user.id);
    ...
    if (!amount || !duration || !pan || !universityName || !course) {
      return res.status(400).json({ error: 'Missing required fields' });
    }

    // Check existing applications
    const existingLoans = await LoanApplication.findAll({ where: { userId: req.user.id } });

    // Rule: Max 2 applications
    if (existingLoans.length >= 2) {
      return res.status(400).json({ error: 'Maximum application limit (2) reached.' });
    }

    // Rule: If applying for 2nd loan, 1st must have documents
    if (existingLoans.length === 1) {
      const existingDocs = await StudentDocument.findAll({ where: { userId: req.user.id } });
      if (existingDocs.length === 0) {
        return res.status(400).json({ error: 'Please upload documents for your first application before applying for a new one.' });
      }
    }

    const applicationId = `EDU-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`;
    ...
    const newLoan = await LoanApplication.create({
      ...
      purpose,
      pan: pan.toUpperCase(),
      cibilScore: cibilScore ? parseInt(cibilScore) : null,
      status: 'Pending',
      applicantCountry: applicantCountry || null,
      applicantIntake: applicantIntake || null,
      admissionStatus: admissionStatus || null,
      appliedDate: new Date()
    });

    // Also update profile status if first loan
    ...
    // Log activity
    await logAccountActivity('loan_application_submitted', {
      userId: user.id,
      email: user.email,
      metadata: {
        applicationId: newLoan.applicationId,
        amount: newLoan.amount,
        university: newLoan.universityName
      }
    });
...
app.get('/api/loans/my-applications', authenticateToken, async (req, res) => {
  try {
    ...
    if (user.role === 'student') {
      console.log(`[API] Fetching loans for user: ${user.email} (ID: ${user.id}, Role: ${user.role})`);
      const myLoans = await LoanApplication.findAll({
        where: { userId: req.user.id },
        order: [['createdAt', 'DESC']]
      });
      console.log(`[API] Found ${myLoans.length} existing loans.`);

      // If no applications, seed the initial one from Lead data
      if (myLoans.length === 0) {
        console.log(`[API] Attempting to seed application for user: ${user.email}`);
        // Try to find the lead for this user to get real data
        const lead = await StudentLead.findOne({ where: { email: user.email } });

        let initialAmount = 2500000; // Default fallback
        let initialUniversity = 'Technical University of Munich'; // Default fallback
        let initialCountry = 'Germany';
        let initialIntake = 'Sep 2024';
        let initialAdmissionStatus = 'Applied';

        if (lead) {
          console.log(`[API] Found lead (ID: ${lead.id}) for seeding.`);
          if (lead.universityPreference) {
            initialUniversity = lead.universityPreference;
          }
          if (lead.loanRange) {
            // Parse loan range e.g. "₹21 - ₹30 lacs" -> 2100000
            const match = lead.loanRange.match(/(\d+)/);
            if (match && match[0]) {
              initialAmount = parseInt(match[0]) * 100000;
            }
          }
          if (lead.studyCountry) initialCountry = lead.studyCountry;
          if (lead.intake) initialIntake = lead.intake;
          if (lead.admissionStatus) initialAdmissionStatus = lead.admissionStatus;
        } else {
          console.log('[API] No lead found for user, using defaults.');
        }

        const applicationId = `EDU-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`;

        try {
          const initialLoan = await LoanApplication.create({
            userId: req.user.id,
            applicationId,
            universityName: initialUniversity,
            course: 'Graduate Course', // Default as not captured in lead
            ...
            cibilScore: 750,
            status: 'Pending',
            applicantCountry: initialCountry,
            applicantIntake: initialIntake,
            admissionStatus: initialAdmissionStatus,
            appliedDate: new Date()
          });
          console.log(`[API] Successfully seeded initial loan: ${initialLoan.id}`);
          return res.json({ applications: [initialLoan] });
        } catch (seedError) {
          console.error('[API] Error seeding initial loan:', seedError);
          // Return empty list if seeding fails so UI doesn't break, but log error
...
// Email sending endpoint for admins
app.post('/api/admin/send-email', authenticateToken, async (req, res) => {
  try {
    // Check if user is admin
    const user = await User.findByPk(req.user.id);
    if (!user || user.role !== 'admin') { // Assuming 'admin' role, adjust if 'super_admin' only
...
app.get('/api/loans/all', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const mappedLoans = rows.map(loan => {
      ...
      return {
        ...loanData,
        studentName: loanData.user ? loanData.user.name : 'Student',
        email: loanData.user ? loanData.user.email : '',
        phone: loanData.user ? loanData.user.phone : '',
        // Ensure applicationId exists, fallback only if needed
        applicationId: loanData.applicationId || `LA-2024-${loanData.id}`
      };
    });
...
app.put('/api/loans/:id/approve', authenticateToken, async (req, res) => {
  ...
  // Log activity
  await logAccountActivity('loan_application_approved', {
    userId: req.user.id, // Admin who approved
    email: req.user.email,
    metadata: {
      applicationId: loanApplications[index].applicationId,
      studentId: loanApplications[index].userId
    }
  });
  ...
  try {
    ...
    const loan = await LoanApplication.findByPk(id, {
      include: [{
        model: User,
        as: 'user',
        attributes: ['name', 'email', 'phone', 'country']
      }]
    });
    ...
    // Authorization check: Admin can see all, Student can only see their own
    if (user.role !== 'admin' && user.role !== 'super_admin' && loan.userId !== user.id) {
    ...
    const formattedLoan = {
      ...loanData,
      studentName: loanData.user ? loanData.user.name : 'Unknown Student',
      studentEmail: loanData.user ? loanData.user.email : '',
      phone: loanData.user ? loanData.user.phone : '',
      applicantCountry: loanData.applicantCountry || (loanData.user ? loanData.user.country : ''),
      applicationId: loanData.applicationId || `LA-2024-${loanData.id}`
    };
...
app.get('/api/admin/notifications', authenticateToken, async (req, res) => {
  ...
  // Return activities as notifications
  // In a real app, you might want to filter by read/unread status
  res.json({
    notifications: activities,
    unreadCount: activities.length // For demo purposes, all are treated as unread
  });
});
...
app.get('/api/admin/activities', authenticateToken, async (req, res) => {
  try {
    ...
    const { count, rows } = await AccountActivityLog.findAndCountAll({
      order: [['createdAt', 'DESC']],
      limit: parseInt(limit),
      offset: (parseInt(page) - 1) * parseInt(limit)
    });
...
app.post('/api/auth/verify-email', async (req, res) => {
  try {
    ...
    try {
      const leadRecord = await StudentLead.findOne({ where: { email: user.email.toLowerCase() } });
      if (leadRecord) {
        leadRecord.status = 'converted';
        leadRecord.convertedUserId = user.id;
        leadRecord.convertedAt = new Date();
        await leadRecord.save();
      }
    } catch (leadUpdateError) {
      console.error('⚠️ Failed to update lead status after verification:', leadUpdateError);
    }
...
// Get current user info with status
...
// Mark verification message as seen
app.post('/api/auth/mark-verification-seen', authenticateToken, async (req, res) => {
  try {
    const user = await User.findByPk(req.user.id);
    if (user) {
      // For now, we'll just return success
      // In future, you can add a hasSeenVerificationMessage field to User model
      res.json({ success: true });
    } else {
      res.status(404).json({ error: 'User not found' });
    }
  } catch (error) {
    console.error('Mark verification seen error:', error);
...
app.post('/api/documents/upload', authenticateToken, upload.single('document'), async (req, res) => {
  try {
    ...
    let docRecord = await StudentDocument.findOne({
      where: {
        userId: req.user.id,
    ...
    if (docRecord) {
      ...
      docRecord.filePath = req.file.path;
      docRecord.fileUrl = fileUrl;
      docRecord.category = category;
      docRecord.status = 'Pending'; // Reset status on re-upload
      await docRecord.save();
    } else {
      // Create new
      docRecord = await StudentDocument.create({
        userId: req.user.id,
        documentId,
        category,
    ...
    if (!user.documentsSubmitted) {
      // Ideally check if ALL mandatory docs are uploaded, but simple flag for now
...
app.get('/api/documents/my-documents', authenticateToken, async (req, res) => {
  try {
    ...
    // Transform to map { documentId: { ...data } } for easier frontend consumption
    const docMap = {};
    documents.forEach(doc => {
      docMap[doc.documentId] = {
...
async function startServer() {
  try {
    ...
    console.error('   2. Database "kubera_db" does not exist');
