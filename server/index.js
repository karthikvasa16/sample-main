...
app.post('/api/leads', async (req, res) => {
  try {
    ...
    if (!fullName || !email || !studyCountry || !admissionStatus || !intake || !loanRange) {
    ...
    const [lead, created] = await StudentLead.findOrCreate({
    ...
    if (!created) {
      ...
      lead.intake = intake;
      lead.universityPreference = universityPreference || lead.universityPreference;
...
app.post('/api/leads/:id/send-verification', authenticateToken, requireAdminOrSuper, async (req, res) => {
  try {
    ...
    try {
      await emailService.sendVerificationEmail(user.email, user.name || user.email, verificationToken);
    } catch (emailError) {
